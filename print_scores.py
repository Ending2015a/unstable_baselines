...
if __name__ == '__main__':
  ...
  parser.add_argument('--glob', type=str, default='*.json', help='file to glob')

  a = parser.parse_args()

  json_files = sorted([x for x in Path(a.path).rglob(a.glob)])
  j_pairs = []
  ...
  j_pairs = sorted(j_pairs, key=lambda x:x[1]['monitor']['episode_rewards'], reverse=True)
